...
final int STARTING_SENSOR_ID = 14;
final int PX_PER_MM = 5;
final float PHYSICAL_OFFSET = 35;  // I'm using 35mm as the temporary offset between all the sensors. This will be variable in actuality
...
void setup () {
  ...
  for (int i = 0; i < SENSOR_COUNT; i++) {
    ...
    Sensors.add(new HallSensor(i, PHYSICAL_OFFSET, lastSensor, (int)pxOffset, height / 3));
    pxOffset += PHYSICAL_OFFSET * PX_PER_MM;
  }
  ...
  {  // If data is available,
    ...
    if (input != null) {      
      ...
      if (index != -1) {
        ...
        HallSensor thisSensor = Sensors.get(this_id - STARTING_SENSOR_ID);
